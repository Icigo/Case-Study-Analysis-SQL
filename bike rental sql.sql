...
/* 1. Display the category name and the number of bikes the shop owns in
...
/*
2. For each customer, display the customer's name and the count of
...
*/
SELECT c.name AS customers_name, COUNT(m.id) AS membership_count 
FROM customer c
...
/*
3. For each bike, display its ID, category, old price per hour (call this column 
...
/*
4. Display the number of available bikes (call this column 
   available_bikes_count ) and the number of rented bikes (call this column  rented_bikes_count ) 
   by bike category.
*/
SELECT category AS bike_category, 
...
/*
5. Display the total revenue from rentals for each month, the total for each
...
/*
6. Display the year, the month, the name of the membership type (call this
...
EXTRACT(MONTH FROM m.start_date) AS month, mt.name AS membership_type_name, 
...
/*
7. Display the total revenue from memberships purchased in 2023 for each
